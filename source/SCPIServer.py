        ...
        try:
            with conn:
                ...
                parser = SCPIParser({
                    ...
                    ":MEASure:COUPling:FREQuency": lambda freq: setattr(new_measurement, 'freq', float(freq)) if (config['properties']['freq']['min'] is None or float(freq) >= config['properties']['freq']['min']) and (config['properties']['freq']['max'] is None or float(freq) <= config['properties']['freq']['max']) else None, # technically the bounds cannot be None as the client side validation with the instrument class might fail
